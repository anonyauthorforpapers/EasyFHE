...
# pkl_path = "/data/YOUNAME/data/encode_20250612_171807.pkl"
...
DIRECT_LOAD = True
pre_encode_type = "middle"
pkl_path = "/data/YOUNAME/data/encode_20250721_205409.pkl"
...
def initial_layer(input, cryptoContext):
    ...
    res = fhe.homo_rescale(res, 1, cryptoContext)  # RESCALE ADD BY XXX
...
def layer1(input, cryptoContext):
    ...
    res1 = fhe.homo_rescale(res1, 1, cryptoContext)  # RESCALE ADD BY XXX
    ...
    if cryptoContext.rescaleTech == "FIXEDMANUAL":
        input = fhe.drop_last_elements(input, input.cur_limbs - res1.cur_limbs,
                                       cryptoContext)  # drop_last_elements ADD BY XXX
    ...
    res2 = fhe.homo_rescale(res2, 1, cryptoContext)  # RESCALE ADD BY XXX
    ...
    res2 = fhe.homo_bootstrap(res2, cryptoContext.L - drop, logBsSlots_list[0], levelBudget_list[0], cryptoContext)
    ...
    res3 = fhe.homo_rescale(res3, 1, cryptoContext)  # RESCALE ADD BY XXX
    ...
    if cryptoContext.rescaleTech == "FIXEDMANUAL":
        res2 = fhe.drop_last_elements(res2, res2.cur_limbs - res3.cur_limbs,
                                      cryptoContext)  # drop_last_elements ADD BY XXX
    A2 = read_values_from_file(f"layer{3}-conv{2}bn{2}-A2", cryptoContext.L - res2.cur_limbs, res2.slots, cryptoContext,
                               scale)
...
def layer2(input, cryptoContext):
    ...
    res1sx0 = fhe.homo_rescale(res1sx0, 1, cryptoContext)  # RESCALE ADD BY XXX
    res1sx1 = fhe.homo_rescale(res1sx1, 1, cryptoContext)  # RESCALE ADD BY XXX

    if cryptoContext.rescaleTech == "FIXEDMANUAL":
        boot_in = fhe.drop_last_elements(boot_in, 2, cryptoContext)  # RESCALE ADD BY XXX
    ...
    fullpackSx = fhe.homo_rescale(fullpackSx, 1, cryptoContext)  # RESCALE ADD BY XXX
    ...
    res1 = fhe.homo_bootstrap(res1, cryptoContext.L-drop, logBsSlots_list[0], levelBudget_list[0], cryptoContext)
    ...
    if cryptoContext.rescaleTech == "FIXEDMANUAL":
        res1 = fhe.drop_last_elements(res1, res1.cur_limbs - res2.cur_limbs,
                                      cryptoContext)  # drop_last_elements ADD BY XXX
    A2 = read_values_from_file(f"layer{5}-conv{2}bn{2}-A2", cryptoContext.L - res1.cur_limbs, res1.slots, cryptoContext,
                               scale)
    A2y = fhe.homo_mul_pt(res1, A2, cryptoContext)
    res2 = fhe.homo_add(res2, A2y, cryptoContext)
    res2 = fhe.homo_rescale(res2, 1, cryptoContext)  # RESCALE ADD BY XXX
    ...
    if cryptoContext.rescaleTech == "FIXEDMANUAL":
        res2 = fhe.drop_last_elements(res2, res2.cur_limbs - res3.cur_limbs,
                                      cryptoContext)  # drop_last_elements ADD BY XXX
    A2 = read_values_from_file(f"layer{6}-conv{2}bn{2}-A2", cryptoContext.L - res2.cur_limbs, res2.slots, cryptoContext,
                               scale)
    A2y = fhe.homo_mul_pt(res2, A2, cryptoContext)
    res3 = fhe.homo_add(res3, A2y, cryptoContext)
    res3 = fhe.homo_rescale(res3, 1, cryptoContext)  # RESCALE ADD BY XXX
    res3 = fhe.homo_bootstrap(res3, cryptoContext.L-1, logBsSlots_list[0], levelBudget_list[0], cryptoContext)
...
def layer3(input, cryptoContext):
    ...
    res1sx1 = fhe.homo_rescale(res1sx1, 1, cryptoContext)  # RESCALE ADD BY XXX
    if cryptoContext.rescaleTech == "FIXEDMANUAL":
        boot_in = fhe.drop_last_elements(boot_in, 2, cryptoContext)  # drop_last_elements ADD BY XXX
    ...
    A2 = read_values_from_file(f"layer{9}-conv{2}bn{2}-A2", cryptoContext.L - res2.cur_limbs, res2.slots, cryptoContext,
                               scale)  # drop_last_elements ADD BY XXX
    A2y = fhe.homo_mul_pt(res2, A2, cryptoContext)
    res3 = fhe.homo_add(res3, A2y, cryptoContext)
    res3 = fhe.homo_rescale(res3, 1, cryptoContext)  # RESCALE ADD BY XXX
...
def executeResNet20(cryptoContext):
    ...
    # model.to(device)
    # model_path = '/home/YOUNAME/PNP/GPU-FHE/examples/resnet20/gen_aespa_weights/ResNet20_Aespa.pth'
...
def homo_Aespa_perfect_square(x, filename, cryptoContext):
    if x.noise_deg >1:
        x = fhe.homo_rescale(x, 1, cryptoContext) #RESCALE ADD BY XXX
    ...
    perfect_squre = fhe.homo_rescale(perfect_squre, 1, cryptoContext)  # RESCALE ADD BY XXX
