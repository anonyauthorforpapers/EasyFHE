...
def conv_initial(input, img_width, padding, num_channel, scale, cryptoContext):
    ...
    for j in range(num_channel):
        ...
        partial_sum = fhe.homo_rescale(partial_sum, 1, cryptoContext)  # RESCALE ADD BY XXX
...
def convbn_dx(input, num_channel, rot_offset, layer, n, channel_offset, biasoff, scale, cryptoContext):
    ...
    finalsum = fhe.homo_rescale(finalsum, 1, cryptoContext)  # RESCALE ADD BY XXX
...
def convbn_dx_32K(input, num_channel, rot_offset, layer, n, channel_offset, biasoff, num_cipher, scale, cryptoContext):
    ...
    finalsum0 = fhe.homo_rescale(finalsum0, 1, cryptoContext)  # RESCALE ADD BY XXX
    finalsum1 = fhe.homo_rescale(finalsum1, 1, cryptoContext)  # RESCALE ADD BY XXX
...
def downsample1024to256(c1, c2, num_channel, num_cipher, cryptoContext):
    ...
    for cipher in cipher_list:
        if cipher.noise_deg > 1:
            fullpack = fhe.homo_rescale(cipher, 1, cryptoContext)  # RESCALE ADD BY XXX
        else:
            fullpack = cipher
        fullpack = fhe.homo_mul_pt(fhe.homo_add(fullpack,
                                   ...
                                   cryptoContext)
        # 相邻两个相加
        fullpack = fhe.homo_rescale(fullpack, 1, cryptoContext)  # RESCALE ADD BY XXX
        fullpack = fhe.homo_mul_pt(fhe.homo_add(fullpack,
                                                fhe.homo_rotate(
        ...
        fullpack = fhe.force_rescale(fullpack, 1, cryptoContext)  # RESCALE ADD BY XXX
...
def downsample256to64(c1, c2, num_channel, cryptoContext):
    ...
    downsampledrows = fhe.force_rescale(downsampledrows, 1, cryptoContext)  # RESCALE ADD BY XXX
